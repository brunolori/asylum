...
public class ApiExceptionHandler {
    ...
    protected ResponseEntity<?> handleValidationException(ValidationException ex, HttpHeaders headers, HttpStatus status, WebRequest request) {
        ...
        return handleExceptionInternal(ex, new ApiException(ex.getMessage(),null,status.value(),2,"VALIDATION_EXCEPTION",errors), headers, status, request);
    }
    ...
    protected ResponseEntity<?> handleEntityExistsException(EntityExistsException ex, HttpHeaders headers, HttpStatus status, WebRequest request) {
        List<String> errors = Collections.singletonList(ex.getMessage());
        return handleExceptionInternal(ex, new ApiException(ex.getMessage(),null,status.value(),2,"ENTITY_EXISTS_EXCEPTION",errors), headers, status, request);
    }
    ...
    protected ResponseEntity<?> handleInvalidTokenException(InvalidTokenException ex, HttpHeaders headers, HttpStatus status, WebRequest request) {
        List<String> errors = Collections.singletonList(ex.getMessage());
        return handleExceptionInternal(ex,  new ApiException(ex.getMessage(),null,status.value(),3,"INVALID_TOKEN",errors), headers, status, request);
    }

    protected ResponseEntity<?> handleGeneralException(Exception ex, HttpHeaders headers, HttpStatus status, WebRequest request) {
        List<String> errors = Collections.singletonList(ex.getMessage());
        ex.printStackTrace();
        return handleExceptionInternal(ex,  new ApiException("GABIM NE SISTEM",null,status.value(),4,"GENERAL_ERROR",errors), headers, status, request);
    }
